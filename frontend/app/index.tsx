...
// Botones: Iniciar Sesión → /login | Registrarse → /registro
...
export default function WelcomeScreen() {
  ...
  const slideButtons = useRef(new Animated.Value(20)).current;
  const scaleLogo    = useRef(new Animated.Value(0.7)).current;

  useEffect(() => {
    Animated.sequence([
      Animated.spring(scaleLogo, { toValue: 1, tension: 60, friction: 6, useNativeDriver: true }),
      ...
      Animated.parallel([
        ...
        Animated.timing(slideButtons, { toValue: 0, duration: 400, useNativeDriver: true }),
      ]),
    ]).start();
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topSection}>
        <Animated.View style={[styles.logoWrapper, { transform: [{ scale: scaleLogo }] }]}>
          <View style={styles.logoCircle}>
            {/* Reemplazar por logo real cuando esté disponible:
                <Image source={require('../assets/images/usc-logo.png')}
      ...
      {/* Tarjeta inferior */}
      <Animated.View
        style={[styles.bottomCard, { opacity: fadeCard, transform: [{ translateY: slideCard }] }]}
      >
        ...
        <Animated.View
          style={[styles.buttonsContainer, { opacity: fadeButtons, transform: [{ translateY: slideButtons }] }]}
        >
          {/* Botón principal */}
          <TouchableOpacity
...
const styles = StyleSheet.create({
  container: {
    ...
    alignItems: 'center', justifyContent: 'space-between', paddingBottom: 40,
  },
  bgCircle: {
    position: 'absolute', width: 420, height: 420, borderRadius: 210,
    backgroundColor: USC_BLUE_DARK, top: -180, right: -100, opacity: 0.6,
  },
  topSection: {
    flex: 1, alignItems: 'center', justifyContent: 'center',
    paddingTop: 20, paddingHorizontal: 28,
  },
  ...
  logoCircle: {
    ...
    borderWidth: 2, borderColor: 'rgba(255,255,255,0.35)', zIndex: 1,
  },
  logoRingOuter: {
  ...
  appTitle: {
    fontSize: 42, fontWeight: '900', color: '#FFFFFF', letterSpacing: 1.5,
    textShadowColor: 'rgba(0,0,0,0.25)',
    textShadowOffset: { width: 0, height: 2 }, textShadowRadius: 6, marginBottom: 8,
  },
  appSubtitle: {
    fontSize: 13, color: USC_GOLD, textAlign: 'center', fontWeight: '500', letterSpacing: 0.4,
  },
  bottomCard: {
    width: '100%', backgroundColor: 'rgba(255,255,255,0.08)',
    borderTopLeftRadius: 32, borderTopRightRadius: 32,
    borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)',
    paddingHorizontal: 28, paddingTop: 32, paddingBottom: 16,
  },
  description: {
  ...
  loginButton: {
    backgroundColor: USC_GREEN, height: 56, borderRadius: 14,
    flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 10,
    shadowColor: USC_GREEN, shadowOffset: { width: 0, height: 6 },
    shadowOpacity: 0.4, shadowRadius: 12, elevation: 6,
  },
  loginButtonText: { color: '#FFFFFF', fontSize: 18, fontWeight: '700', letterSpacing: 0.5 },
  buttonArrow: { color: '#FFFFFF', fontSize: 20, fontWeight: '300' },
  registerButton: {
    height: 52, borderRadius: 14, borderWidth: 2,
    borderColor: 'rgba(255,255,255,0.4)',
    justifyContent: 'center', alignItems: 'center',
  },
  registerButtonText: { color: '#FFFFFF', fontSize: 16, fontWeight: '600', letterSpacing: 0.5 },
});
